...
private[dao] object ParametersTable {
  ...
  def setLedgerAndParticipantId(ledgerId: String, participantId: String)(
      connection: Connection,
  ): Unit =
    discard(
      SQL"insert into #$TableName(#$LedgerIdColumnName, #$ParticipantIdColumnName) values($ledgerId, $participantId)"
        .execute()(connection)
    )

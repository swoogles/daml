...
class PackageCommitterSpec
  ...
  private[this] val validationModes = Table(
    "package validation mode",
    PackageValidationMode.NoValidation,
    PackageValidationMode.Precommit,
    PackageValidationMode.Postcommit
  )

  "buildLogEntry" should {
    "set record time in log entry if record time is available" in
      forEvery(validationModes) { validationMode =>
        val instance = new PackageCommitter(mock[Engine], validationMode, metrics)
